...
class ZertoClient:
    ...
    async def _get(self, path: str) -> dict | list:
        ...
        # Reintenta una vez si el token expiró entre medias (401)
    ...
    async def get_vpg_vms(self, vpg_identifier: str) -> list:
        """
        VMs protegidas por un VPG concreto. El endpoint exacto puede
        variar; si tu ZVMA no lo expone así, ajusta esta ruta.
        """
        try:
    ...
    async def get_events(self) -> list:
        """
        Endpoint de eventos recientes. No está en la lista "core"
        documentada — si tu versión de Zerto no lo expone en /v1/events,
        esta función simplemente devuelve una lista vacía en vez de romper
        el dashboard.
        """
        try:
