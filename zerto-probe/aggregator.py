...
pinta la pestaña "Zerto" de la Observability Console (ver capturas de
referencia: VPG Status, Actual RPO, Journal History, Failsafe History,
tabla de VMs protegidas, alertas activas y eventos recientes).

Los nombres de campo exactos de `/v1/vpgs` pueden variar ligeramente
entre versiones de Zerto. Aquí se usan varios alias razonables vía
`_first()`; si tu instancia real usa otros nombres, este es el único
fichero que deberías tocar.
"""
...
async def build_site_payload(client: ZertoClient, label: str) -> dict:
    """
    Construye el payload completo de un site (origen o destino),
    combinando VPGs + VMs protegidas + alertas + eventos.
    """
    try:
